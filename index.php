...
$result = mysqli_query($link, "SELECT time FROM online WHERE ip='$ip'");
$time = time();
if($row = mysqli_fetch_assoc($result)){
    mysqli_query($link, "UPDATE online SET time='$time',ip='$ip' WHERE ip='$ip'");
}else{
    mysqli_query($link, "INSERT INTO online (ip,time) VALUES ('$ip','$time')") or die(mysqli_error());
}
$result   = mysqli_query($link, "SELECT ip FROM online");
...
$sorgula = mysqli_query($link, "select tarih from ziyaret where tarih='$bitis'");
...
$bugun = date("Y-m-d");
$sorgu = mysqli_query($link, "select tarih from ziyaret where tarih='$bugun'");
...
$ipkontrol = mysqli_query($link, "select * from ziyaret where ip='$ipsi' order by id desc");
...
if($bak>0){  //if-
    if($vtarih<$tarih){
        $kayit_1 = mysqli_query($link, "insert into ziyaret (ip,tarih) values ('$ipsi','$tarih')");
    }
}//if-
else{
    $kayit_2 = mysqli_query($link, "insert into ziyaret (ip,tarih) values ('$ipsi','$tarih')");
}
$toplamne = mysqli_query($link, "select * from ziyaret");
